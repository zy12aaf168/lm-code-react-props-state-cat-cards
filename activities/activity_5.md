...
❗ Note! Our CatCard currently uses an _implicit return_, i.e. we just straight return JSX from the function. To introduce some of our own code, we have to add some curly braces and then wrap the JSX in a `return ()` statement.

Add the console.log like this:
...
Passing the whole cat object makes `App.tsx` a little tidier. It also means that if we change the cat object then those changes are _automatically_ passed down to the child components!
